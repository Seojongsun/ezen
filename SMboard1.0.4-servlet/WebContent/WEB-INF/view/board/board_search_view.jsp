      ...
      <fieldset>
       ...
       <label for="keyword"></label> 
        ...
        <%--해당 항목을 기본 선택으로 지정하여 검색한다.--%>
         ...
         <option value="subject" <c:if test="${'subject' == sessionScope.keyfield}">selected="selected"</c:if>> 제목 </option>
								<option value="name" <c:if test="${'subject' == sessionScope.keyfield}">selected="selected"</c:if>>작성자 </option>
								<option value="content" <c:if test="${'subject' == sessionScope.keyfield}">selected="selected"</c:if>> 내용</option>
        </select>
    ...
    <div class="col-md-12">
     <div class="card">
      <div class="card-header">
       <h3 class="text-muted"><i class="fas fa-file-alt mr-sm-1"></i>검색글 목록</h3>
       <p align="right">  
       <%--전체 글의 개수를 호출한다.--%>      
...
<c:if test="${board.writeday == nowDay}">
      ...
      <c:if test="${searchlistcount==0}">
       <tr>
        <td colspan="4"></td>
        <td class="text-center">입력하신 내용으로 검색한 글의 내용은 없습니다.</td>
       </tr>
      </c:if>
      <tr>
      </table>
      ...
      <nav class="ml-4">
       <ul class="pagination justify-content-center">        
         ...
         <c:otherwise>
         <li class="page-item">
         <a href="./BoardSearchSelect.do?page=${page-1}&keyword=${sessionScope.keyword}&keyfield=${sessionScope.keyfield}" class="page-link"> 이전 </a>
          </li>  
         </c:otherwise>
       </c:choose> 
       ...
       <c:forEach var="start" begin="${startpage}" end="${endpage}" step="1">
         ...
         <a class="page-link">${start}</a>
          ...
          <c:otherwise>
          <li class="page-item">
         <a href="./BoardSearchSelect.do?page=${start}&keyword=${sessionScope.keyword}&keyfield=${sessionScope.keyfield}" class="page-link">${start} </a>
        </li>
          </c:otherwise>
         </c:choose>
        </c:forEach> 
        <c:choose>
         ...
         <c:otherwise>
          <a href="./BoardSearchSelect.do?page=${page+1}&keyword=${sessionScope.keyword}&keyfield=${sessionScope.keyfield}">
      ...
      <nav class="navbar justify-content-end">
       ...
       <div class="btn-group">
        <button type="button" class="btn btn-outline-info mr-sm-1" onclick="location.href='./BoardInsertView.do'">
